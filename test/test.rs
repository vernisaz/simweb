...
impl simweb::WebPage for Page {
    ...
    fn apply_specific(
        &self,
        page_map: &mut HashMap<&str, String>,
    ) -> Result<(), Box<dyn std::error::Error + 'static>> {
        page_map.insert("timestamp", simweb::http_format_time(SystemTime::now()));
        page_map.insert(
            "Epoch",
            format!(
                "{}",
                simweb::parse_http_timestamp(page_map.get("timestamp").unwrap()).unwrap()
            ),
        );
        page_map.insert(
            "current_path",
        ...
        page_map.insert(
            "base64",
            base64_encode_with_padding("h75s!!@$#-`".as_bytes()),
        );
...
fn format_time(time: SystemTime, local: bool) -> String {
    ...
    let offset = if local {
        simtime::get_local_timezone_offset() * 60
    } else {
        0
    };
    let (y, m, d, h, min, _s, _w) =
        simtime::get_datetime(1970, ((dur.as_secs() as i64) + offset as i64) as u64);
